 ...
 */
public class CusURLResource implements Resource, RangeAwareResource {
    ...
    @Deprecated
    public CusURLResource(final URL url, URLConnection connection, String path) {
        this(url, path);
    }

    public CusURLResource(final URL url, String path) {
        this.url = url;
        this.path = path;
    }
        ...
        if (!connectionOpened) {
            ...
            try {
                try {
                    ...
                    WebServer.setCommHeader(connection);
            ...
            if (file != null) {
                try (DirectoryStream<Path> stream = Files.newDirectoryStream(file)) {
                    for (Path child : stream) {
                        result.add(new CusURLResource(child.toUri().toURL(), child.toString()));
                    }
                }
            }
        } catch (IOException e) {
